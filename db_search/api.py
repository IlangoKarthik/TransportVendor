...
def get_user_search_engine(user_id):
    ...
    if user_id not in user_search_engines:
        ...
        user_search_engines[user_id] = SemanticSearch(user_id=user_id, auto_load=True)
...
def refresh():
    ...
    try:
        ...
        # Get user-specific search engine
        search_engine = get_user_search_engine(user_id)
        print(f"✓ Got search engine for user {user_id}")
        
        print("🔄 Starting force rebuild...")
        search_engine.initialize(force_rebuild=True)
        
        # Check if embeddings were created
        import os
        embeddings_path = search_engine.vector_store_path
        if os.path.exists(embeddings_path):
            file_size = os.path.getsize(embeddings_path)
            print(f"✅ Embeddings file created: {embeddings_path}")
            print(f"✅ File size: {file_size} bytes")
        else:
            print(f"⚠️  Embeddings file not found: {embeddings_path}")
        
        vendor_count = search_engine.vector_store.get_count()
        print(f"✅ Refresh complete! {vendor_count} vendors indexed")
        print("="*60 + "\n")
        
        return jsonify({
            'success': True,
            'message': f'Index refreshed successfully! {vendor_count} vendors indexed.',
