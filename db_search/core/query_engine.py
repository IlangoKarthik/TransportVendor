...
class SemanticSearch:
    ...
    def __init__(self, data_source: Optional[str] = None, 
        ...
        Args:
            ...
            user_id: User ID for filtering MongoDB vendors (optional)
        ...
        self.data_loader = DataLoader(data_source, user_id=user_id)
        self.text_processor = TextProcessor()
        self.embedding_generator = EmbeddingGenerator()
        self.vector_store = VectorStore()
        
        # Use user-specific cache path if user_id provided
        if user_id:
            import os
            cache_dir = os.path.join('data', 'users', str(user_id), 'embeddings')
            os.makedirs(cache_dir, exist_ok=True)
            self.vector_store_path = os.path.join(cache_dir, 'vendor_embeddings.pkl')
        else:
    ...
    def _build_vector_store(self):
        ...
        if not vendors:
            print("⚠ No vendor data found - initializing empty search engine")
            return
